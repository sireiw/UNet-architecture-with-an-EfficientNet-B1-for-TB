...
def cb_focal_loss(logits, targets, class_counts, gamma=1.5, beta=0.999, device='cuda'):
...
def coral_loss(feat_s, feat_t, shrink=1e-3):
...
@torch.cuda.amp.autocast(enabled=False) 
def mmd_loss(feat_s, feat_t, kernel='rbf', bandwidth=None):
...
def kd_loss(student_logits, teacher_logits, labels=None, T=4.0, alpha=0.7):
